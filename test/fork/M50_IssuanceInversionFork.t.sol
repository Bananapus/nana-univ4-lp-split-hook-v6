...
/// @notice Fork test proving M-50 fix: extreme weight no longer causes
///         precision loss in _getIssuanceRateSqrtPriceX96.
contract M50_IssuanceInversionFork is Test {
    ...
    /// @notice Deploy pool with extreme weight (1e28 tokens/ETH). Before M-50 fix, the intermediate
    ///         division in _getIssuanceRateSqrtPriceX96 would lose significant precision at high rates.
    function test_fork_m50_extremeWeight_noRevert() public {
        // Use very high weight: 1e28 (10 billion tokens per ETH). This exercises the direct
        // sqrt computation path and validates precision at extreme issuance rates.
        ...
        // Pay to create surplus, then accumulate tokens for LP.
        _payProject(pid, 100 ether);
        _accumulateTokens(pid, address(pToken), 1e27);

        // Deploy pool — exercises the M-50 fixed sqrtPrice computation.
        vm.prank(multisig);
