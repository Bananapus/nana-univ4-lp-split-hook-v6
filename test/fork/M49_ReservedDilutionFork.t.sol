...
/// @notice Fork test proving M-49 fix: fallback price reflects reserved% dilution.
///         With 50% reserved, the fallback price should be ~50% of the gross weight price.
contract M49_ReservedDilutionFork is Test {
    ...
    /// @notice Deploy pool with 50% reserved. The pool's initial price should reflect the
    ///         diluted issuance (500k tokens/ETH instead of 1M tokens/ETH).
    function test_fork_m49_reservedDilution_priceReflectsDilution() public {
        // 50% reserved — only 500k tokens/ETH reach the payer, not the gross 1M.
