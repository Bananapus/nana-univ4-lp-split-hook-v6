...
/// @notice Fork test proving M-48 fix: zero-rate guard in _getSqrtPriceX96ForCurrentJuiceboxPrice
///         prevents TickMath revert when weight is extreme or issuance is zero.
contract M48_ZeroSqrtPriceFork is Test {
    ...
    /// @notice Deploy pool with 100% reserved rate (issuance == 0). The fallback price
    ///         now returns a valid extreme price instead of causing TickMath to revert.
    function test_fork_m48_zeroIssuance_noTickMathRevert() public {
        // 100% reserved → issuanceRate == 0.
        ...
        // Deploy pool — exercises the zero-rate guard in _getSqrtPriceX96ForCurrentJuiceboxPrice.
        vm.prank(multisig);
