...
/// @notice Fork test proving M-47 fix: the 50% cash-out cap is removed, allowing
///         the formula-derived amount to be used fully. This results in higher LP liquidity
///         for configurations where the optimal cash-out exceeds 50%.
contract M47_CashOutCapFork is Test {
    ...
    /// @notice Deploy pool with parameters where the optimal cash-out exceeds 50%.
    ///         With the M-47 fix removing the cap, the pool should have higher liquidity.
    function test_fork_m47_noCashOutCap_higherLiquidity() public {
        // High surplus relative to tokens, low cashOutTaxRate → formula wants >50% cash-out.
        uint256 pid = _launchProject({reservedPercent: 0, cashOutTaxRate: 1000, weight: 1_000_000e18});

        vm.prank(multisig);
        IJBToken pToken = jbController.deployERC20For(pid, "No Cap Token", "NCP", bytes32(0));

        // Large surplus relative to accumulated tokens.
        ...
        // Verify the pool state is valid.
