...
- Leftover project tokens after `MINT_POSITION + SETTLE + SWEEP` are carried back into `accumulatedProjectTokens[projectId]` (becoming future liquidity); leftover terminal tokens are carried into `accumulatedTerminalTokens[projectId][terminalToken]` (also becoming future liquidity). The hook never burns leftover and never deposits it into the project's terminal.
...
- Both ledgers are zeroed before any external call (CEI), so a reentrant `addLiquidity` reverts (nothing accumulated); leftovers are carried forward afterward (project-token dust back to `accumulatedProjectTokens`, terminal-token dust back to `accumulatedTerminalTokens`) — never burned, never deposited into the project's terminal.

## A.5 Fee routing (post-deployment)

- `collectAndRouteLPFees` is **permissionless**. Anyone can trigger a fee collection cycle. It collects fees from the single active position (re-ranging burns the old position and re-mints, so there is only ever one position per pair). Each side — project-token and terminal-token — takes a best-effort `feeProjectId` cut (via `_attemptFeeProjectCut`), and the non-cut remainder of each side is carried into this project's own ledger: the project-token remainder into `accumulatedProjectTokens[projectId]`, the terminal-token remainder into `accumulatedTerminalTokens[projectId][terminalToken]`. Neither side is ever deposited into the project's terminal or treasury. No caller receives extracted value.
...
- `_routeCollectedFees` splits the collected pair into its project-token and terminal-token sides and calls `_attemptFeeProjectCut` on EACH side independently — the cut is symmetric, not one-sided. The non-cut remainder is written to its ledger BEFORE any external fee-project `pay` call, for CEI. The hook never burns either fee side.
- The fee-project cut on either side is credited to the fee project's **primary** terminal for that token, not an attacker-chosen one. If the fee project has no primary terminal accepting that token, `_attemptFeeProjectCut` forgives the cut for that side and the full amount is carried into the project's own ledger instead — a misconfigured fee project never blocks or shrinks the project's own fee collection.
- A reverting fee-project `pay` call is forgiven the same way: `_attemptFeeProjectCut` pays through a try/catch self-call (`payFeeProjectCut`) and, on failure, fully rolls back any pre-incremented reserve and carries the whole amount into the project's own ledger. Fee collection itself can never fail because of the fee project's state.
...
- The fee project's returned ERC-20 amount is measured via balance delta, not the `pay()` return value, whenever the fee project has an ERC-20 deployed. Fee-on-transfer or nonstandard fee-project terminals cannot overstate what the hook actually received.
...
- Rebalance is structurally `collect fees → recompute corridor → drift + TWAP guards → burn old position → re-mint one new position`. Fees are collected first so each side's non-cut remainder lands in this project's own ledger (ask-leg `accumulatedProjectTokens`, bid-leg `accumulatedTerminalTokens`) and folds into the re-mint, rather than being read twice — once as fees, once as burn principal. The corridor itself is derived from the project's rates and surplus, which fee routing leaves untouched: collected LP fees become protocol-owned liquidity held in the hook, never treasury surplus.
...
- Leftovers after re-mint: project-token leftovers are carried back into `accumulatedProjectTokens`; terminal-token leftovers are carried back into `accumulatedTerminalTokens[projectId][terminalToken]`. Nothing is burned and nothing is deposited into the project's terminal balance. Conservation holds modulo on-chain rounding.
...
- For the fee project's terminal during the best-effort fee-cut payment (`payFeeProjectCut`), the hook calls `_requireTemporaryAllowanceConsumed` after the external `pay` call for ERC-20 fee tokens. A fee terminal that does not consume the full pre-call allowance reverts with `JBUniswapV4LPSplitHook_TemporaryAllowanceNotConsumed` — caught and forgiven by the surrounding best-effort wrapper rather than blocking collection, so a leaked approval never sits open for an attacker to exploit later.
...
- **No direct token withdrawal.** There is no `rescueTokens` or `withdraw` function. The hook's only paths out are: pay a best-effort `feeProjectId` cut to the fee project's terminal via `terminal.pay`, or mint into / increase a V4 position. The hook never cashes out, never burns project tokens, and never deposits into the project's own terminal. Stuck tokens (e.g. accidentally-sent foreign ERC-20s) cannot be recovered by the operator.
...
- A missing or reverting fee-project terminal does NOT block the project's own fee inflow: `_attemptFeeProjectCut` forgives the cut (whether from a missing terminal or a reverting `pay`) and the entire collected amount for that side is carried into the project's own ledger (`accumulatedProjectTokens` or `accumulatedTerminalTokens`) instead.
...
- **`deployPool(projectId)`** — permissionless. Selects the highest-ETH-value terminal token via `findHighestValueTerminalTokenOf`. Sets `hasDeployedPool[projectId] = true` before any external calls. Initializes or accepts a pre-initialized pool (validated against economic tick bounds; the oracle-TWAP guard applies to an already-initialized pool). Mints a single-sided ask of the accumulated project tokens from the live spot up to the issuance ceiling via Permit2 + V4 `MINT_POSITION + SETTLE + SWEEP` — no cash-out. Carries project-token leftover back into `accumulatedProjectTokens[projectId]` and terminal-token leftover into `accumulatedTerminalTokens[projectId][terminalToken]` (never burns, never deposits into the project's terminal).
  - **Invariants:** one-shot per `(projectId, terminalToken)`; one-shot per project (only one terminal token supported per project); economic spot-below-ceiling gate; pre-init pool rejected if outside or AT `[floorTick, ceilingTick]`; zero-liquidity reverts atomically; leftover carried to the project's own ledgers, never burned, never deposited into the project's terminal.

- **`addLiquidity(projectId, terminalToken)`** — permissionless. Requires a deployed pool and accumulation ≥ `_MIN_ADD_ACCUMULATION`. Reverts on TWAP unavailability or spot/TWAP deviation > `_MAX_TWAP_DEVIATION_TICKS`. Collects + routes the active position's fees (each side taking a best-effort `feeProjectId` cut, non-cut remainder to this project's own ledger), burns the prior position to recover its principal, and re-mints a single fresh adaptive position at the live corridor (folding in `accumulatedProjectTokens` + recovered principal + recovered terminal + `accumulatedTerminalTokens` + claimed credits). No cash-out; any terminal-token bid leg comes only from the terminal recovered by burning this project's own position plus this project's own `accumulatedTerminalTokens` ledger. Carries leftovers forward (never burns project tokens, never deposits into the project's terminal). Emits `LiquidityAdded`.
...
- **`rebalanceLiquidity(projectId, terminalToken)`** — permissionless, bounded by the corridor-drift threshold (`_MIN_REBALANCE_DRIFT_TICKS` on at least one bound) and the TWAP guard (the same as `addLiquidity`, since the re-mint prices against the live spot). Collects + routes fees first (each side taking a best-effort `feeProjectId` cut, non-cut remainder to this project's own ledger); recomputes the corridor; burns the old position with a contract-derived slippage floor; re-mints one new position from per-project snapshot balance deltas plus `accumulatedTerminalTokens`. Reverts if recomputed liquidity is zero.
...
- **`collectAndRouteLPFees(projectId, terminalToken)`** — anyone. Requires `tokenIdOf[projectId][terminalToken] != 0`. Calls `_collectAndRouteFees` which uses `DECREASE_LIQUIDITY(0) + TAKE_PAIR` to fetch only accrued fees from the single active position; `_routeCollectedFees` then takes a best-effort `feeProjectId` cut on EACH side (project-token and terminal-token) via `_attemptFeeProjectCut`, carrying every non-cut remainder into this project's own ledger — `accumulatedProjectTokens[projectId]` for the project-token side, `accumulatedTerminalTokens[projectId][terminalToken]` for the terminal-token side. Neither side is ever deposited into the project's terminal.
  - **Invariants:** principal liquidity never removed; ledger writes for the non-cut remainder (a state write) precede the external fee-project `pay` call (reentrancy ordering); routed amounts measured by balance delta, not return values; a missing or reverting fee-project terminal forgives that side's cut rather than stranding the project's share.

**Receive / interfaces:**

- **`receive() payable`** — accepts ETH for native-terminal fee-project cut payments (`payFeeProjectCut`) and V4 `TAKE`/`SWEEP` operations.
- **`supportsInterface(bytes4)`** — `IJBUniswapV4LPSplitHook` and `IJBSplitHook`.

**Public state (selected, all read-only via auto-getters):**

- `accumulatedProjectTokens[projectId]` — the ask-leg reserved-token escrow ledger (pre- AND post-deploy); zeroed before each deploy/add/rebalance mint and re-credited with any leftover/carry and any non-cut project-token fee remainder.
- `accumulatedTerminalTokens[projectId][terminalToken]` — the per-project bid-leg terminal-token escrow ledger; credited only from this project's own recovered burn-terminal, its own mint leftovers, and its own non-cut terminal-token fee remainder — never from another project's balance or the shared clone's raw terminal-token balance. Zeroed before each mint and re-credited the same way as the project-token ledger.
...
4. **State-then-call ordering in fee routing.** `_attemptFeeProjectCut` pre-increments `_totalOutstandingFeeTokenClaims` and `_inflightFeeRoutingCount` BEFORE calling the fee project's terminal `pay()` (via the self-called `payFeeProjectCut`), then reconciles after — or fully rolls back both reserves on the forgive/catch path. A reentrant collection during the external call sees an inflated reserve, which conservatively prevents over-spending.
...
8. **No direct withdrawal.** The hook holds tokens only to (a) accumulate for LP in `accumulatedProjectTokens` / `accumulatedTerminalTokens`, (b) pay a best-effort protocol fee cut to the fee project's terminal, (c) mint into / increase a V4 position. The hook never cashes out and never deposits into the project's own terminal. There is no `rescue` / `withdraw` / `recover` function, and no project-token burn path. Misrouted foreign tokens cannot be extracted by the operator.
...
- **Fee-project owner.** Configures the fee-project terminal. A misconfigured fee-project terminal (missing primary terminal for the collected token) silently forfeits the fee cut on that side, and the full amount is carried into the project's own ledger (`accumulatedProjectTokens` or `accumulatedTerminalTokens`) instead; this is documented in RISKS.md §7.2 and §7.5.
