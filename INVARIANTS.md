...
- Before the deploy/add funding cash-out, the hook claims any hook-held project credits into the project's ERC-20 and includes those tokens in the LP sizing. This keeps Juicebox's credit-first burn order from leaving transferable project tokens outside the accumulation ledger.
...
- Any hook-held credits for the project are claimed into ERC-20s before `addLiquidity` computes and performs its funding cash-out, so externally sent credits are treated as additional project-token principal rather than an alternate burn source that can desync the ledger.
