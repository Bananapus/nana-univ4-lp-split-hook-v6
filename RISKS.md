...
- **`claimFeeTokensFor`** -- Requires `SET_BUYBACK_POOL` permission. Fee tokens accumulate in `claimableFeeTokens[projectId]` and are only withdrawable by the project owner or operator. If unclaimed, tokens sit in the hook contract indefinitely.
...
- **Fee tokens match actual balance** -- `claimableFeeTokens[projectId]` should equal the actual fee project token balance attributable to that project. After `claimFeeTokensFor`, balance is zeroed before transfer, preventing reentrancy double-claim.
