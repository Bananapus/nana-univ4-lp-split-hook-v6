...
- **Reentrancy trust boundary during `deployPool` (SI-006)** -- `deployPool` calls `_deployPoolAndAddLiquidity`, which writes `_poolKeys` (line 1129) and `tokenIdOf` (line 878) before `deployedPoolCount` is incremented (line 586). During the `_mintPosition` call inside `_addUniswapLiquidity`, external contracts (`POSITION_MANAGER`, and indirectly `IJBMultiTerminal` via `cashOutTokensOf`) execute callbacks. If any of these contracts were compromised and re-entered `processSplitWith`, tokens would be accumulated (via `_accumulateTokens`) and then cleared when `accumulatedProjectTokens` is zeroed (line 884) -- meaning re-entrantly added tokens would be lost. This is accepted because the Juicebox protocol's security model already treats `POSITION_MANAGER`, `IJBMultiTerminal`, and `IJBController` as trusted, non-malicious contracts. The `processSplitWith` entry point itself is gated to `msg.sender == controllerOf(projectId)` (line 598), so only the JB controller can invoke it. Additionally, the `tokenIdOf != 0` guard (line 568) prevents re-entering `deployPool` once the position is minted, and the `deployedPoolCount == 0` branch in `processSplitWith` (line 604) will route to accumulation only before the first pool is deployed. The practical reentrancy surface requires a compromised core protocol contract, which is outside this hook's threat model.
...
- **Spot price usage in rebalance (NM-001)** -- `rebalanceLiquidity` uses `POOL_MANAGER.getSlot0()` (lines 708, 800), which returns the instantaneous spot price, not a time-weighted average (TWAP). This means the liquidity calculation is sensitive to the pool price at the exact moment the transaction executes. **Operational advice for project owners:**
